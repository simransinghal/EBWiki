...
RSpec.describe CasesHelper, :type => :helper do
  ...
  # These specs confirm that the case_sanity_check works as expected
  ...
  describe "#case_sanity_check" do
    ...
    it "returns false if any of the instance variables needed for the case are not present" do
      this_case = FactoryGirl.create(:case, community_action: 'community text')
      this_case.state = FactoryGirl.create(:state)
      assign(:case, this_case)
      assign(:commentable, this_case)
